    ...
    <tr>
      ...
      {foreach from=$res item=row name=main}
        ...
        <tr>
          ...
          <td class="{$tdClass}"><input type="button" name="editproject{$row.id}" value="Szerkesztés" onclick="javascript: document.location='admin.php?mod=productsedit&id={$row.id}';" class="admin-button" /></td>
        </tr>    
    {/foreach}
  </table>
</form>
